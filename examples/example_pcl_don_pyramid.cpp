...
int main(int argc, char *argv[])
{
  ...
  desc.add_options()
          ...
          ("modelfile", po::value<string>(&modelfile)->required(), "the file to read a point cloud model from")
  ...
  int histogramheight = std::ceil((radius2-radiusincrement-radius1)/radiusincrement);
  int histogramwidth = std::ceil((radius2-radius1+radiusincrement)/radiusincrement);
  //total votes to normalize by
  int votes = 0;

  vector< vector<float> > histogram = vector < vector<float> >(histogramheight);
  for(unsigned int i = 0; i < histogram.size(); i++){
    histogram[i].resize(histogramwidth);
  }
  ...
  for(float scale2 = radius1+radiusincrement; scale2 < radius2; scale2 += radiusincrement){
    for(float scale1 = radius1; scale1 < scale2; scale1+= radiusincrement){
      ...
      for (size_t point_id = 0; point_id < doncloud->points.size (); ++point_id)
      {
        ...
        if(mag > 0.75){
          int indexh = (int)((scale2 - radius1 - radiusincrement)/radiusincrement);
          int indexw = (int)((scale1 - radius1)/radiusincrement);
          cout << indexh << ", " << indexw << endl;
          histogram[indexh][indexw]++;
          votes++;
        }
      }
      std::cout << scale1 << ", "
          << scale2 << ", "
          << boost::accumulators::extract::min(acc_mag) << ", "
          << boost::accumulators::extract::max(acc_mag) << ", "
          << boost::accumulators::extract::count(acc_mag) << ", "
          << sum(acc_mag) << ", "
          << median(acc_mag) << ", "
          << mean(acc_mag) << ", "
          << variance(acc_mag) << std::endl;
    }

    std::cout << std::endl;
  }

  for(vector< vector<float> >::iterator i = histogram.begin(); i!= histogram.end(); i++){
